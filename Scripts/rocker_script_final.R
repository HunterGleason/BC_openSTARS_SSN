...
ssn_dir <- paste("/home/rstudio/BC_openSTARS_SSN/SSN/",readline(prompt="Please enter the desired name of the output SNN object, e.g., bowron.ssn: "),sep="")

#Path to DEM 
dem_path<-"/home/rstudio/BC_openSTARS_SSN/DataUTM10/parsnip_utm.tif"

#Path to observed stream sites.
sites_path <- "/home/rstudio/BC_openSTARS_SSN/DataUTM10/trophic_2020_metrics.shp"
...
stream_path<-"/home/rstudio/BC_openSTARS_SSN/DataUTM10/stream.shp"

#Vector of predictive raster attribute layers, and Abbrv. vector 
#Enter path to gridded air temp
r_pred_paths<- c("/home/rstudio/BC_openSTARS_SSN/DataUTM10/trophic_2020_at.awat [Universal Kriging].tif",
                 "/home/rstudio/BC_openSTARS_SSN/DataUTM10/trophic_2020_at.awcoef [Universal Kriging].tif",
                 "/home/rstudio/BC_openSTARS_SSN/DataUTM10/trophic_2020_at.mwat [Universal Kriging].tif")

#Make raster covar names shp file friendly 
r_pred_names<-c('aAwat',
                'aAwcv',
                'aAwmt')
...
print("Calculating flow accumulation from DEM ...")
execGRASS("r.watershed",flags = c("overwrite"),
          parameters = list(
            elevation = dem_name,
            drainage = "flow_acc"
...
calc_attributes_edges(input_raster = c('dem','slope','gradt_ds','aAwat','aAwcv','aAwmt'), 
                      stat_rast = c('mean','mean','mean', 'mean','mean','mean'), 
                      attr_name_rast = c('avEle','avSlo','avGrdt','avAwat','avAwcv','avAwmt'),
                      round_dig = 5
)
...
calc_attributes_sites_approx(sites_map = "preds_o", 
                             input_attr_name = c('avEle','avSlo','avGrdt','avAwat','avAwcv','avAwmt'),
                             output_attr_name = c('avEleA','avSloA','avGrdtA','avAwatA','avAwcvA','avAwmtA'),
